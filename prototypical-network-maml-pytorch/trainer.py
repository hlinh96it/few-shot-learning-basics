...
def train_model(model_class, train_loader: DataLoader, val_loader: DataLoader, proto_dim=64, learning_rate=2e-4):
    trainer = pl.Trainer(default_root_dir=os.path.join(CHECKPOINT_PATH, model_class.__name__),
                         accelerator='mps', devices=1, max_epochs=10, enable_progress_bar=True,
...
def test_model(model_class: torch.nn.Module, test_loader: DataLoader):
    trainer = pl.Trainer(default_root_dir=os.path.join(CHECKPOINT_PATH, model_class.__name__,),
                         accelerator='mps', devices=1, enable_progress_bar=True)
    pretrained_filename = os.path.join(CHECKPOINT_PATH, model_class.__name__ + '.ckpt')
    while not os.path.isfile(pretrained_filename):
        pretrained_filename = input(f'Not found pretrained model {pretrained_filename}, new dir: ')
