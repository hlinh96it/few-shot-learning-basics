...
if __name__ == '__main__':
    ...
    cifar_all_images = np.concatenate(
        [cifar_train_set.data, cifar_test_set.data], axis=0)
    cifar_all_labels = torch.LongTensor(
        cifar_train_set.targets + cifar_test_set.targets)

    # %% Split data to train and test, original we have 100 classes
    # random 100 numbers with range of 0 to 99 ~ shuffle
    ...
    train_set = dataset_from_labels(
        cifar_all_images, cifar_all_labels, train_classes, img_transform=train_transforms)
    validation_set = dataset_from_labels(
        cifar_all_images, cifar_all_labels, val_classes, img_transform=test_transforms)
    test_set = dataset_from_labels(
        cifar_all_images, cifar_all_labels, test_classes, img_transform=test_transforms)
